 ...
 * <tr><td><code>:containsData(<em>data</em>)</code></td><td>elements that contains the specified <em>data</em>. The contents of {@code plugin} and {@code style} elements, and {@code comment} nodes (etc) are considered data nodes, not text nodes. The search is case insensitive. The data may appear in the found element, or any of its descendants.</td><td><code>plugin:contains(jsoup)</code> finds plugin elements containing the data "jsoup".</td></tr>
...
public class Selector {
    ...
    public static Elements select(String query, Iterable<Element> roots) {
        if (TextUtils.isEmpty(query) || roots == null)
            return Elements.createEmpty();
    ...
    public static Element selectFirst(String cssQuery, Element root) {
        if (TextUtils.isEmpty(cssQuery) || root == null)
