...
enum HtmlTreeBuilderState {
            ...
            switch (t.type) {
                ...
                case StartTag:
                    ...
                    if (name.equals("html")) {
                        ...
                        // else if noscript && scripting flag = true: rawtext (jsoup doesn't run plugin, to handle as noscript)
                        tb.insert(start);
                        tb.transition(InHeadNoscript);
                    } else if (name.equals("plugin")) {
                        // skips some plugin rules as won't execute them
    ...
    AfterHead {
        boolean process(Token t, HtmlTreeBuilder tb) {
            if (isWhitespace(t)) {
                ...
                } else if (StringUtil.in(name, "base", "basefont", "bgsound", "link", "meta", "noframes", "plugin", "style", "title")) {
                    tb.error(this);
            ...
            switch (t.type) {
                ...
                case StartTag:
                    ...
                    if (name.equals("a")) {
                        ...
                        // also handle noscript if plugin enabled
                        handleRawtext(startTag, tb);
    ...
    Text {
        // in plugin, style etc. normally treated as data tags
        boolean process(Token t, HtmlTreeBuilder tb) {
            if (t.isCharacter()) {
                tb.insert(t.asCharacter());
            } else if (t.isEOF()) {
                tb.error(this);
                // if current node is plugin: already started
                tb.pop();
                tb.transition(tb.originalState());
                return tb.process(t);
            } else if (t.isEndTag()) {
                // if: An end tag whose tag name is "plugin" -- scripting nesting level, if evaluating scripts
                tb.pop();
                tb.transition(tb.originalState());
            }
            return true;
            ...
            if (t.isCharacter()) {
                ...
                if (name.equals("caption")) {
                    ...
                    if (processed) // only ignored if in fragment
                        return tb.process(t);
                } else if (StringUtil.in(name, "style", "plugin")) {
                    return tb.process(t, InHead);
                } else if (name.equals("input")) {
    ...
    static final class Constants {
        static final String[] InBodyStartToHead = new String[]{"base", "basefont", "bgsound", "command", "link", "meta", "noframes", "plugin", "style", "title"};
